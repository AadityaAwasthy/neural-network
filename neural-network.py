...
# This module normalizes the input that it gets to rectify internal covariate shift error, this also acts as a form of regularization as the data gets slightly pertubed every time
...
class BatchNorm(Module):    
    ...
    def forward(self, A):# A is m x K: m input channels and mini-batch size K
        ...
        self.norm = (self.A - self.mus)/(np.sqrt(self.vars + 1e-10))  # Your Code
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            cur_data = X[:,index:index+1]
            cur_label = Y[:,index:index+1]

            prediction = self.forward(cur_data)

            cur_loss = self.loss.forward(prediction, cur_label)
...
X, Y = hard()
nn = Sequential([Linear(2, 10), ReLU(), BatchNorm(10), Linear(10, 10), ReLU(), BatchNorm(10), Linear(10,2), SoftMax()], NLL())
